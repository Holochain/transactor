...
var dna = function (exports) {
  ...
  */
	function trace() {
		...
		debug('@@ [trace] ' + args.map(function (i) {
    ...
    */
			...
			value: function _getInt(oth) {
				if (oth instanceof UnitValue) {
					...
					var numInt = parseInt(oth);
					if (numInt !== oth) {
						throw new Error('number has sub-integer component');
					}
					var tmp = numInt.toString(16);
 ...
 */
  ...
  */
	function fixTime(t) {
		...
		var m = t.match(/(\d{4})-(\d{2})-(\d{2})\s*(\d{2}):(\d{2}):(\d{2})(?:\.\d+)*\s*([+-])(\d{2})(\d{2})/);
		if (m && m.length === 10) {
